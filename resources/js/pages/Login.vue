  ...
  <section class="section login">
    <div class="container">
      <div class="login__inner">
        <form @submit.prevent="login" class="form-login text-center needs-validation"
          v-bind:class="{ novalidate: emptyFields }">
          <h4 class="heading mb-5 pb-3">{{ heading }}</h4>
          <div class="form-group">
            <input type="email" v-model="emailLogin" class="form-control" :placeholder="emailPlaceholder"
              autocomplete="off" required>
            <span class="icon focus__form-control"><font-awesome-icon :icon="['fas', 'envelope']" /></span>
          </div>
          <div class="form-group mt-5">
            <input type="password" v-model="passwordLogin" class="form-control" :placeholder="passPlaceholder"
              autocomplete="off" required>
  ...
  methods: {
    async login() {
      if (this.emailLogin === '' || this.passwordLogin === '') {
        ...
        this.$store.dispatch('helper/sha256', { password: this.passwordLogin }).then(
          (response) => {
            let user = {
